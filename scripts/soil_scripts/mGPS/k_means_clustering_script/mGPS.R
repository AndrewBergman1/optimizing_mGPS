...
# Description:
#   Run mGPS using Yali's distance calculation (if classified country == original country, distance = 0, else distance = distance to nearest border).
...
# Calling this function as proxy for haversine_distance function
...
# Pulls an observation to the nearest land border
...
run_mGPS_cv <- function(dataset, dataset_name) {
  ...
  clusters <- unique(dataset$Cluster)
  ...
  for (cluster in clusters) {
    ...
    #fold <- folds[[i]]
    train <- dataset[dataset$Cluster != cluster, ]
    test <- dataset[dataset$Cluster == cluster, ]
    ...
    preds <- mGPS(
      ...
      classTarget = "Country",
      nthread = 1,
      hierarchy = c("Continent", "Cluster", "Latitude", "Longitude"),
      variables = variables
    )
    ...
    counts_within_thresholds <- sapply(distance_thresholds, function(threshold) sum(distance_km <= threshold))
    ...
    cat("Confusion Matrix for", cluster, ":\n")
    print(cm$table)
    cat("\n")
  }
  ...
  for (cluster in clusters) {
    fold_cm <- cm_values[[cluster]]
    write.csv(fold_cm$table, paste0("confusion_matrix_", dataset_name, "_fold_", cluster, ".csv"), row.names = TRUE)
  }
...
# Attribute geography if missing (country, continent)
...
# Calculate the distance to nearest country border for Yali's distance calculation
...
# Load Your Datasets
interpolation_smote <- read.csv("~/Master/15hp_project/mGPS-master/mGPS-master/soil_scripts/serious_time/3.5_cluster_data/round_2/smoted_interpolated_clustered_samples.csv")
smote <- read.csv("~/Master/15hp_project/mGPS-master/mGPS-master/soil_scripts/serious_time/3.5_cluster_data/round_2/interpolated_clustered_samples.csv")
original <- read.csv("~/Master/15hp_project/mGPS-master/mGPS-master/soil_scripts/serious_time/3.5_cluster_data/round_2/original_dataset.csv")
...
interpolation_smote$Cluster <- as.factor(interpolation_smote$Cluster)
smote$Cluster <- as.factor(smote$Cluster)
original$Cluster <- as.factor(original$Cluster)
