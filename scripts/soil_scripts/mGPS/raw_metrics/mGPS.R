...
# Description:
#   Run mGPS using raw distance calculation. The actual euclidean distance is calculated between the predicted and actual coordiantes.
...
# feature selection
species_select <- function(x, y, remove_correlated = TRUE, cores = 1, maxRuns = 10000, confidence = 0.01) {
  set.seed(1)
...
# Pulls prediction to nearest land border
...
run_mGPS_cv <- function(dataset, dataset_name) {
  ...
  # Create 5 random folds for cross-validation
  folds <- createFolds(dataset$Country, k = 5, list = TRUE, returnTrain = FALSE)
  ...
  for (i in 1:length(folds)) {
    ...
    features <- species_select(
      x = train[!names(train) %in% c("Country", "Continent", "Latitude", "Longitude", "Cluster")],
      y = train$Country,
      maxRuns = 100,
      cores = 1
    )
    ...
    cm_values[[paste0("Fold_", i)]] <- cm
    
    # Compute the Haversine Distance Between Predicted and Actual Coordinates
    ...
    distance_km <- haversine_distance(actual_lat, actual_lon, predicted_lat, predicted_lon)
    
    # Optionally, set distance to 0 where classification is correct
    # correct_classification <- pred_countries == test$Country
    # distance_km[correct_classification] <- 0
    
    # Use distance_km as your error metric
    errors <- c(errors, distance_km)
    
    # Tag Errors with Their Corresponding Fold
    fold_tags <- c(fold_tags, paste0("Fold_", i))
    
    # Calculate Classification Accuracy
    num_matches <- sum(pred_countries == test$Country)
    ...
    counts_within_thresholds <- sapply(distance_thresholds, function(threshold) sum(distance_km <= threshold, na.rm = TRUE))
  ...
  for (i in 1:length(folds)) {
    fold_cm <- cm_values[[paste0("Fold_", i)]]
  ...
  write.csv(as.data.frame(t(distance_counts_overall)), paste0("distance_metrics_", dataset_name, "_overall_counts.csv"), row.names = FALSE)
  write.csv(as.data.frame(t(distance_percentages_overall)), paste0("distance_metrics_", dataset_name, "_overall_percentages.csv"), row.names = FALSE)
...
# Load Your Datasets
x2_interpolation_smote <- read.csv("~/Master/15hp_project/mGPS-master/mGPS-master/soil_scripts/serious_time/1_SMOTE/iterations/2x_interpol/interpolated_smoted_dataset.csv")
interpolation_smote <- read.csv("~/Master/15hp_project/mGPS-master/mGPS-master/soil_scripts/serious_time/1_SMOTE/round_2/interpolated_smoted_dataset.csv")
smote <- read.csv("~/Master/15hp_project/mGPS-master/mGPS-master/soil_scripts/serious_time/3_attribute_geo/round_2/interpolated_dataset_w_countries.csv")
original <- read.csv("~/Master/15hp_project/mGPS-master/mGPS-master/soil_scripts/serious_time/3_attribute_geo/round_2/original_dataset.csv")

x2_interpolation_smote$Continent <- as.factor(x2_interpolation_smote$Continent)
...
x2_interpolation_smote$Country <- as.factor(x2_interpolation_smote$Country)
...
if ("X" %in% names(interpolation_smote)) {
  interpolation_smote <- interpolation_smote %>% select(-X)
}
if ("X" %in% names(smote)) {
  smote <- smote %>% select(-X)
}
if ("X" %in% names(original)) {
  original <- original %>% select(-X)
}
if ("X" %in% names(x2_interpolation_smote)) {
  x2_interpolation_smote <- x2_interpolation_smote %>% select(-X)
}
...
datasets <- list(
  ...
  smote = smote,
  original = original,
  x2_interpolation_smote = x2_interpolation_smote
)
